...
// Entrena el modelo SVM concurrentemente usando el algoritmo de margen máximo 
func (svm *SVM) trainConcurrent(data [][]float64, labels []float64, epochs int, learningRate float64, lambda float64, wg *sync.WaitGroup) {
	defer wg.Done()
...
func main() {
	...
	go svm.trainConcurrent(data, labels, 100, 0.01, 0.001, &wg)
	wg.Wait()
	...
	fmt.Printf("True Positive: %d, True Negative: %d\nFalse Positive: %d, False Negative: %d\n\n", tp, tn, fp, fn)
